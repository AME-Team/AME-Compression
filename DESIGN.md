...
- **Apple レベルの品質** — すべての UI コンポーネントは、Apple 製品と同等の洗練さと使いやすさを目指す。
...
1024px 以下の画面では `font-size: 14px` に縮小。
...
4. Electron メインプロセスは IPC 経由で背景色を同期（`setThemeColor`）
   - ダーク: `#111827`
   - ライト: `#f9fafb`
...
```css
box-shadow: 0 0 0 2px var(--color-primary), 0 0 0 4px rgba(168, 85, 247, 0.2);
```
...
- **キーボードナビゲーション**: `CommandPalette` は矢印キー対応、`ComparisonSlider` は矢印キー対応
- **ARIA ラベル**: 各コンポーネントに適切な ARIA 属性を設定
