...
- **ノイズ除去**。設定可能なレベル（0.0〜1.0）の `afftdn` フィルター。
  UI側の値（0.0〜1.0）は、`nr = int(denoise_level * 97)` に変換してFFmpegの `nr` パラメータ（0〜97 dB）にマッピングされます（`backend/volume.py:build_audio_filter()` を参照）。
...
## 13. AI レビュー＆修正ループプロトコル (AI REVIEW & ITERATION PROTOCOL)

開発ワークフローにおいて品質と堅牢性を最大化するため、以下のループ手順を必ず遵守してください：

1. **プルリクエスト（PR）の作成**: 実装が完了したら、PRを作成します。
2. **AI レビューの実行**: AI（Gemini Code Assistなど）によるコードレビューおよびサジェスチョンを確認します。
3. **指摘の修正**: レビューで提示されたすべての指摘事項、バグの懸念、リファクタリングの提案に対して誠実に対応・修正します。
4. **再 Push と再レビュー**: 修正をコミット・プッシュし、再度レビューを受けます。
5. **完了条件**:
   - レビューによる指摘やサジェスチョンが完全になくなるまで、この修正ループを繰り返します。
